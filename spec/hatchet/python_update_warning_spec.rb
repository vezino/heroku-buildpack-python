...
RSpec.describe 'Python update warnings' do
  ...
  context 'with a runtime.txt containing python-3.9.0' do
    let(:allow_failure) { false }
    let(:app) { Hatchet::Runner.new('spec/fixtures/python_3.9_outdated', allow_failure: allow_failure) }
    ...
    context 'when using Heroku-22', stacks: %w[heroku-22] do
      ...
      # Whilst Python 3.9 is supported on Heroku-22, only the latest version (3.9.12) has been built.
      # TODO: Once newer Python 3.9 versions are released, adjust this test to use 3.9.12,
